...
int main (){
	...
	printf("\t\t\t\t\t      MASUKAN PIN ANDA:"); scanf("%d", &pin);
		if (nokartu == nokartu1 && pin == fpin1) {
			...
			printf("NO KARTU/PIN YANG ANDA MASUKAN SALAH!\n");
			i=i+1;
			getch();
			goto login;
			system("cls");
	...
	switch(y1) { 
		...
		case '6': { // Keluar Dari Program
			printf("***** EXIT *****\n");
			printf("TERIMA KASIH TELAH MENGGUNAKAN ATM TUBES");
			goto exit;
			break;
		}
...
void getDataNasabah () { //Data Nasabah
	printf ("Nama			: Hilma\n");
	printf ("NIK			: 3271046504930002\n");
	printf ("No Rekening		: 001 123 4567\n");
	printf ("Jenis Kartu		: Silver\n");
}
void getMenu() { // Menu utama
	printf("| 				***** MENU UTAMA *****				|\n");
	printf("| 1. TRANSFER DAN PEMBAYARAN		4. MUTASI TRANSAKSI			|\n");
...
void getMenuPilihTransfer(){ // Menu Pilihan Transfer Dana
	printf("		****** TRANSFER DANA ******			\n");
	printf("* 1. Transfer Dana ke Rekening Bank TUBES	     	*\n");
